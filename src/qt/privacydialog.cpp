...
void PrivacyDialog::on_pushButtonZPrxControl_clicked()
{
    ZPrxControlDialog* zPrxControl = new ZPrxControlDialog(this);
    zPrxControl->setModel(walletModel);
    zPrxControl->exec();
}

void PrivacyDialog::setZPrxControlLabels(int64_t nAmount, int nQuantity)
{
...
void PrivacyDialog::sendzPRX()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid Prius Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " PRX </b>will be added to the standard transaction fees!<br />";
    ...
    if (!ZPrxControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZPrxControlDialog::GetSelectedMints();
    }
    ...
    // Clear zprx selector in case it was used
    ZPrxControlDialog::listSelectedMints.clear();
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " Prx, ";
