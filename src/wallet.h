...
enum ZerocoinSpendStatus {
    ZPRX_SPEND_OKAY = 0,                            // No error
    ZPRX_SPEND_ERROR = 1,                           // Unspecified class of errors, more details are (hopefully) in the returning text
    ZPRX_WALLET_LOCKED = 2,                         // Wallet was locked
    ZPRX_COMMIT_FAILED = 3,                         // Commit failed, reset status
    ZPRX_ERASE_SPENDS_FAILED = 4,                   // Erasing spends during reset failed
    ZPRX_ERASE_NEW_MINTS_FAILED = 5,                // Erasing new mints during reset failed
    ZPRX_TRX_FUNDS_PROBLEMS = 6,                    // Everything related to available funds
    ZPRX_TRX_CREATE = 7,                            // Everything related to create the transaction
    ZPRX_TRX_CHANGE = 8,                            // Everything related to transaction change
    ZPRX_TXMINT_GENERAL = 9,                        // General errors in MintToTxIn
    ZPRX_INVALID_COIN = 10,                         // Selected mint coin is not valid
    ZPRX_FAILED_ACCUMULATOR_INITIALIZATION = 11,    // Failed to initialize witness
    ZPRX_INVALID_WITNESS = 12,                      // Spend coin transaction did not verify
    ZPRX_BAD_SERIALIZATION = 13,                    // Transaction verification failed
    ZPRX_SPENT_USED_ZPRX = 14,                      // Coin has already been spend
    ZPRX_TX_TOO_LARGE = 15                          // The transaction is larger than the max tx size
};
...
public:
    ...
    */
    ...
    void setZPrxAutoBackups(bool fEnabled)
    {
