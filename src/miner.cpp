...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("PriusMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("PriusMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("PriusMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("prius-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrintf("Running PriusMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
